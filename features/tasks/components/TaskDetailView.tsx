...
import { createTaskComment, deleteTaskAction, submitTaskAction, updateTaskChecklistItemAction, updateTaskStatus } from "@/features/tasks/actions/taskActions";
      ...
      <section style={shell}>
        <div style={header}>
          <PageHeader
            eyebrow="업무 상세"
            title={task.title}
            description={`담당 ${assigneeNames} / 생성 ${task.creator.name}`}
            meta={
              <div style={metaLine}>
            ...
            actions={
              <div className="asc-action-group">
                <ButtonLink href="/tasks" variant="secondary" size="sm">업무 목록</ButtonLink>
                {canDelete && (
        ...
        <div style={grid}>
              ...
              task.status === "DONE" ? (
                <>
                  ...
                  <form action={updateTaskStatus} style={submitForm}>
                    ...
                    <Button type="submit" variant="secondary" size="sm">완료 취소</Button>
                  </form>
                </>
              ) : (
                <form action={submitTaskAction} style={submitForm}>
                  ...
                  <Button type="submit" size="sm">완료</Button>
                </form>
              )
            ) : (
            ...
            <div style={list}>
              {task.checklistItems.map((item) => (
                <form key={item.id} action={updateTaskChecklistItemAction} style={checkRow}>
                  ...
                  {canWork && task.status !== "DONE" && <Button type="submit" variant="secondary" size="sm">저장</Button>}
                </form>
              ))}
          ...
          <Panel title="진행 메모">
            <form action={createTaskComment} style={commentForm}>
              ...
              <Button type="submit" size="sm">메모 저장</Button>
            </form>
            <div style={list}>
...
const summaryCard: CSSProperties = { background: "var(--asc-surface)", border: "1px solid transparent", borderRadius: "var(--asc-radius-lg)", padding: 10, display: "grid", gap: 4, boxShadow: "var(--asc-shadow-sm)" };
const grid: CSSProperties = { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 };
const panel: CSSProperties = { background: "var(--asc-surface)", border: "1px solid transparent", borderRadius: "var(--asc-radius-lg)", padding: 12, display: "grid", gap: 10, alignContent: "start", boxShadow: "var(--asc-shadow-sm)" };
const panelTitle: CSSProperties = { margin: 0, fontSize: 16, fontWeight: 950 };
const infoRow: CSSProperties = { display: "grid", gridTemplateColumns: "110px 1fr", gap: 10, borderBottom: "1px solid var(--asc-border-subtle)", padding: "8px 0", fontSize: 13 };
const descriptionBox: CSSProperties = { whiteSpace: "pre-wrap", lineHeight: 1.6, color: "var(--asc-text-subtle)", background: "var(--asc-bg-subtle)", border: "1px solid transparent", borderRadius: "var(--asc-radius-md)", padding: 10 };
const list: CSSProperties = { display: "grid", gap: 8 };
const checkRow: CSSProperties = { display: "grid", gridTemplateColumns: "1fr auto auto", gap: 8, alignItems: "center", borderBottom: "1px solid var(--asc-border-subtle)", padding: "6px 0" };
...
const textarea: CSSProperties = { width: "100%", border: "1px solid transparent", borderRadius: "var(--asc-radius-md)", padding: 9, background: "var(--asc-bg-subtle)", color: "var(--asc-text)", resize: "vertical" };
const primaryButton: CSSProperties = { height: 34, border: "1px solid transparent", borderRadius: "var(--asc-radius-md)", background: "var(--asc-primary)", color: "#fff", padding: "0 12px", fontWeight: 950 };
const dangerButton: CSSProperties = { ...primaryButton, background: "var(--asc-danger-soft)", color: "var(--asc-danger)" };
const commentForm: CSSProperties = { display: "grid", gap: 8 };
const commentItem: CSSProperties = { display: "grid", gap: 4, border: "1px solid transparent", borderRadius: "var(--asc-radius-md)", padding: 10, background: "var(--asc-bg-subtle)" };
const timeline: CSSProperties = { display: "grid", gap: 10 };
const timelineItem: CSSProperties = { display: "grid", gridTemplateColumns: "16px 1fr", gap: 10, borderBottom: "1px solid var(--asc-border-subtle)", paddingBottom: 10 };
const timelineDot: CSSProperties = { width: 10, height: 10, borderRadius: "var(--asc-radius-sm)", background: "var(--asc-primary)", marginTop: 6 };
...
const holdBadge: CSSProperties = { ...badge, background: "var(--asc-primary-soft)", color: "var(--asc-primary)" };
...
const empty: CSSProperties = { border: "1px dashed var(--asc-border-subtle)", borderRadius: "var(--asc-radius-md)", padding: 14, textAlign: "center", color: "var(--asc-text-muted)", fontWeight: 900, background: "var(--asc-bg-subtle)" };
