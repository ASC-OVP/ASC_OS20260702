...
import { ClassGroupStatus, MemoType, Prisma, StudentStatus, StudentTestScoreSource } from "@prisma/client";
...
export async function updateCounselingRecord(formData: FormData) {
  const user = await requireUser();
  ...
  if (!recordId || !date || !content) {
    throw new Error("수정할 상담 날짜와 내용을 입력해 주세요.");
  }

  const record = await prisma.counselingRecord.findFirst({
    where: {
      id: recordId,
      academyId: user.academyId,
      ...(studentId ? { studentId } : {}),
  ...
  await prisma.counselingRecord.update({
    where: { id: record.id },
    data: { date, title, content, status },
  });
  ...
  await recordActivity({
    ...
    entityType: "CounselingRecord",
    entityId: record.id,
    summary: `상담 기록 수정: ${title}`,
    metadata: { studentId: record.studentId, date, status },
  });
...
export async function deleteCounselingRecord(formData: FormData) {
  const user = await requireUser();
  ...
  if (!recordId) {
    throw new Error("삭제할 상담 기록을 찾을 수 없습니다.");
  }
  ...
  await prisma.counselingRecord.delete({ where: { id: record.id } });
...
export async function updateClinicRecord(formData: FormData) {
  const user = await requireUser();
  ...
  if (!recordId || !date || !title) {
    throw new Error("수정할 클리닉 날짜와 제목을 입력해 주세요.");
  }

  const record = await prisma.clinicRecord.findFirst({
    where: {
      id: recordId,
      academyId: user.academyId,
      ...(studentId ? { studentId } : {}),
  ...
  if (!record) {
    throw new Error("클리닉 기록을 찾을 수 없습니다.");
  }

  await prisma.clinicRecord.update({
    where: { id: record.id },
    data: { date, title, content, status },
  });
...
export async function deleteClinicRecord(formData: FormData) {
  const user = await requireUser();
  ...
  if (!recordId) {
    throw new Error("삭제할 클리닉 기록을 찾을 수 없습니다.");
  }
  ...
  await prisma.clinicRecord.delete({ where: { id: record.id } });
...
export async function updateQuestionRecord(formData: FormData) {
  const user = await requireUser();
  ...
  const answer = nullableText(formData, "answer");
  const status = detailStatusValue(text(formData, "status"), answer ? "ANSWERED" : "OPEN");
  const subject = formData.has("subject") ? nullableText(formData, "subject") : undefined;

  if (!recordId || !date || !content) {
    throw new Error("수정할 질문 날짜와 내용을 입력해 주세요.");
  }

  const record = await prisma.questionRecord.findFirst({
    where: {
      id: recordId,
      academyId: user.academyId,
      ...(studentId ? { studentId } : {}),
  ...
  if (!record) {
    throw new Error("질문 기록을 찾을 수 없습니다.");
  }

  await prisma.questionRecord.update({
    where: { id: record.id },
    data: {
      date,
      content,
...
export async function deleteQuestionRecord(formData: FormData) {
  const user = await requireUser();
  ...
  if (!recordId) {
    throw new Error("삭제할 질문 기록을 찾을 수 없습니다.");
  }
  ...
  await prisma.questionRecord.delete({ where: { id: record.id } });
