import { OmrTemplateType } from "@prisma/client";

export type OmrQuestionKind = "CHOICE" | "SHORT";
export type OmrAnswerFormat = "CHOICE" | "SHORT" | "WRITTEN" | "MIXED";
...
export type OmrTemplateQuestion = {
  ...
  defaultSecondaryType?: string;
  defaultDifficulty?: string;
  defaultAnswerFormat?: OmrAnswerFormat;
...
type QuestionDefaults = Partial<Omit<OmrTemplateQuestion, "no" | "section" | "kind">>;

function range(start: number, end: number, section: string, kind: OmrQuestionKind = "CHOICE", defaults: QuestionDefaults = {}): OmrTemplateQuestion[] {
  const defaultAnswerFormat: OmrAnswerFormat = kind === "SHORT" ? "SHORT" : "CHOICE";
